...
사용자가 GitHub Issue에 “로그인이 안됩니다”라고만 쓰면 에이전트는 바로 코드를 고치지 않습니다. 이슈가 너무 짧고 기대 동작이나 재현 조건이 부족하기 때문입니다.

1. 사용자가 Issue를 만들고 에이전트 처리 대상 label을 붙입니다.
2. 에이전트가 Issue 댓글에 분석 결과와 질문을 남깁니다.
3. 사용자는 제시된 선택지를 고를 수 있습니다.

```text
@agent A
```

4. 제시된 선택지가 맞지 않으면 사용자가 직접 방향을 적을 수 있습니다.

```text
@agent 방향: 기존 로그인 정책은 유지하고, 실패 메시지만 사용자가 이해하기 쉽게 바꿔주세요.
```

5. 에이전트가 branch와 PR을 만들면 사용자는 PR의 “사용자가 먼저 확인할 것”을 기준으로 테스트합니다.
6. 통과하면 연결된 Issue에 결과를 남깁니다.
