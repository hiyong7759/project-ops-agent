...
일반 요청자와 검토자는 아래 네 가지를 알면 됩니다.

1. Issue를 만들 때 요청, 현재 증상, 기대 결과, 확인 방법을 적습니다.
2. 에이전트에게 맡길 Issue에는 `agent:queued` label을 붙입니다.
...
Issue는 GitHub/GitLab의 **Issues** 화면에 등록하는 작업 요청 글입니다. Label은 Issue에 붙이는 상태 표시입니다. 사용자가 직접 붙이는 시작 label은 보통 `agent:queued`이며, 이는 “이 Issue를 에이전트 처리 대상으로 올린다”는 뜻입니다.
...
에이전트가 만드는 branch는 `agent/...` prefix를 사용하며 에이전트가 소유합니다.

- 권한 오류처럼 branch push 이후 PR/MR 생성 전에 실패할 수 있습니다.
- 운영자가 원인을 해결하고 같은 Issue에서 재시도하면 에이전트는 원격 기본 branch 기준으로 작업 branch를 다시 만들고 자신의 `agent/...` branch를 갱신합니다.
- 사람이 `agent/...` branch에 직접 수정한 내용은 재시도 때 보존되지 않을 수 있습니다.
- 사람이 직접 보완하려면 별도 branch를 만들거나 PR/MR 리뷰 코멘트로 남깁니다.
